...
private fun MatchChallengesPanel(auth:String,open:(ChatDto)->Unit){
    ...
    LaunchedEffect(auth){try{refresh()}catch(_:Exception){status="تعذر تحميل حالة التطابق"}}
    ...
    when(match.state){
        ...
        "MATCHED" -> Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text("تم العثور على تطابق 🎉",fontWeight=FontWeight.Bold);Text("عدد اللاعبين: ${match.players.size.coerceAtLeast(match.chat?.members?.size?:0)}");Button(onClick={match.chat?.let{{open(it)}}?:{}},enabled=match.chat!=null,modifier=Modifier.fillMaxWidth()){Text("دخول الجلسة")};match.sessionId?.let{id->OutlinedButton(onClick={scope.launch{try{chatApi.leaveMatch(auth,id);match=MatchStatusDto();status=""}catch(_:Exception){status="تعذر مغادرة الجلسة"}}},modifier=Modifier.fillMaxWidth()){Text("مغادرة التطابق")}}}}
            ...
            Text("اختر نوع التطابق، والنظام يجمعك تلقائيًا مع لاعبين متاحين بدون شرط الصداقة.")
            Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text("⚔️ فردي 1 VS 1",fontWeight=FontWeight.Bold);Text("لاعب ضد لاعب داخل جلسة مؤقتة للمحادثة والتحديات.");Button(onClick={scope.launch{busy=true;try{match=chatApi.matchQueue(auth,MatchQueueRequest("ONE_V_ONE"));status=""}catch(_:Exception){status="تعذر بدء البحث"};busy=false}},enabled=!busy,modifier=Modifier.fillMaxWidth()){Text("ابدأ 1 ضد 1")}}}
            Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text("👥 جماعي 5–10",fontWeight=FontWeight.Bold);Text("يبدأ عند توفر 5 لاعبين، ويضم حتى 10 حسب الموجودين.");Button(onClick={scope.launch{busy=true;try{match=chatApi.matchQueue(auth,MatchQueueRequest("GROUP"));status=""}catch(_:Exception){status="تعذر بدء البحث الجماعي"};busy=false}},enabled=!busy,modifier=Modifier.fillMaxWidth()){Text("ابدأ جماعي")}}}
        }
    }
...
private fun RoomsPanel(auth:String,me:UserDto?,open:(ChatDto)->Unit){
    ...
    Text("رومات عامة للسوالف؛ تدخل بدون ما تكون صديقًا للموجودين.")
    ...
    if(showCreate){
        Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
            ...
            Button(onClick={scope.launch{creating=true;try{val result=chatApi.createRoom(auth,CreateRoomRequest(name.trim(),description.trim().ifBlank{null}));name="";description="";showCreate=false;refresh();open(result.chat)}catch(_:Exception){status="تعذر إنشاء الروم"};creating=false}},enabled=!creating&&name.trim().length>=2,modifier=Modifier.fillMaxWidth()){Text("إنشاء ودخول")}
        }}
    }
    if(rooms.isEmpty()) Text("لا توجد رومات عامة حاليًا. تقدر تكون أول من ينشئ روم.")
    rooms.forEach{room->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp),verticalArrangement=Arrangement.spacedBy(6.dp)){
...
private fun ChatRoomScreen(auth:String,me:UserDto?,chat:ChatDto,onBack:()->Unit){
    ...
    DisposableEffect(chat.id){
        ...
        socket.on("typing:changed",typingListener);socket.connect()
    ...
    Column(Modifier.fillMaxSize(),verticalArrangement=Arrangement.spacedBy(8.dp)){
        ...
        Column(Modifier.weight(1f).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(6.dp)){
            messages.forEach{message->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(9.dp)){
