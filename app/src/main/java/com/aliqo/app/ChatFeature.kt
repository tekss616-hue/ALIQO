...
fun ChatsScreen(auth:String,me:UserDto?){
    ...
    selected?.let{opened->
        ChatRoomScreen(auth,me,opened.chat,opened.roomId,opened.roomCreator){selected=null}
        return
    }
...
private fun ChatRoomScreen(auth:String,me:UserDto?,chat:ChatDto,roomId:String?,roomCreator:Boolean,onBack:()->Unit){
    ...
    var sentTyping by remember{mutableStateOf(false)}
    ...
    fun updateTyping(newText:String){
        text=newText.take(4000)
        typingJob?.cancel()
        if(text.isBlank()){if(sentTyping){socketRef?.emit("typing:stop",org.json.JSONObject().put("chatId",chat.id));sentTyping=false};return}
        if(!sentTyping){socketRef?.emit("typing:start",org.json.JSONObject().put("chatId",chat.id));sentTyping=true}
