...
fun NotificationsScreen(auth:String,onUnreadChanged:(Int)->Unit={}){
    ...
    suspend fun refresh(){
        ...
        items=fresh
        NotificationsCache.items=fresh
        PersistentUiCache.saveNotifications(context,"notifications",fresh)
        resolved=true
        onUnreadChanged(fresh.count{it.readAt==null})
    }

    LaunchedEffect(auth){
        if(resolved)onUnreadChanged(items.count{it.readAt==null})
        try{refresh();status=""}catch(_:Exception){resolved=true;if(items.isEmpty()&&!hadStored)status="تعذر تحميل التنبيهات"}
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(10.dp)){
        ...
        if(resolved&&items.isEmpty()) Text("لا توجد تنبيهات حتى الآن")
        items.forEach{item->
