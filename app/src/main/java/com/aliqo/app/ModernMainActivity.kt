...
private fun ModernMainShell(
    ...
    fun go(newTab: String) {
        openedRoomChat = null
        if (newTab != "match") challengeArena = false
        tab = newTab
    }
    ...
    Scaffold(
        ...
        bottomBar = {
            if (!(tab == "match" && challengeArena)) {
                NavigationBar(
                    containerColor = if (darkShell) Color(0xFF081126) else MaterialTheme.colorScheme.surface,
                    tonalElevation = if (darkShell) 0.dp else NavigationBarDefaults.Elevation
                ) {
                    ...
                    NavigationBarItem(
                        selected = tab == "home",
                        onClick = { go("home") },
                        icon = { AliqoArenaIcon(AliqoIcon.HOME, size = 31.dp, active = tab == "home") },
                        label = { Text("الرئيسية") },
                    ...
                    NavigationBarItem(
                        ...
                        onClick = { go("friends") },
                        icon = { AliqoArenaIcon(AliqoIcon.FRIENDS, size = 31.dp, active = tab == "friends") },
                        label = { Text("الأصدقاء") },
                    ...
                    NavigationBarItem(
                        ...
                        icon = {
                            Box {
                                AliqoArenaIcon(AliqoIcon.BELL, size = 31.dp, active = tab == "notifications")
                    ...
                    NavigationBarItem(
                        ...
                        onClick = { go("profile") },
                        icon = { AliqoArenaIcon(AliqoIcon.PROFILE, size = 31.dp, active = tab == "profile") },
                        label = { Text("الملف") },
                        colors = navItemColors(darkShell, selectedOther)
                    )
                }
        ...
        Box(
            Modifier.fillMaxSize().padding(padding)
                .background(if (darkShell) homeBackground else MaterialTheme.colorScheme.background)
        ) {
            when (tab) {
                "home" -> ApprovedHomeDashboard(
                    ...
                    unread = unread,
                    onMatch = { tab = "match" },
                    onRooms = { openedRoomChat = null; tab = "rooms" },
                ...
                "rooms" -> {
                    ...
                    if (chat == null) {
                        PremiumRoomsScreen(auth, me) { opened, id, creator ->
                            openedRoomChat = opened
                            openedRoomId = id
                            openedRoomCreator = creator
                        }
                    } else {
                        RoomConversationScreen(auth, me, chat, openedRoomId, openedRoomCreator) {
                            openedRoomChat = null
                            openedRoomId = null
                            openedRoomCreator = false
                        }
                    }
                }
...
private fun navItemColors(dark: Boolean, indicator: Color): NavigationBarItemColors =
    NavigationBarItemDefaults.colors(
        selectedIconColor = if (dark) Color.White else MaterialTheme.colorScheme.onSecondaryContainer,
        selectedTextColor = if (dark) Color.White else MaterialTheme.colorScheme.onSurface,
        indicatorColor = indicator,
        unselectedIconColor = if (dark) Color(0xFFAAB5D2) else MaterialTheme.colorScheme.onSurfaceVariant,
        unselectedTextColor = if (dark) Color(0xFFAAB5D2) else MaterialTheme.colorScheme.onSurfaceVariant
    )
