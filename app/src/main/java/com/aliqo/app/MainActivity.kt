        ...
        when (section) {
            ...
            "blocked" -> {
                ...
                blocked.forEach { user ->
                    UserCard(user, "إلغاء الحظر", { scope.launch { try { api.unblockUser(auth, user.id); status = "تم إلغاء حظر @${user.username}"; refreshData() } catch (e: Exception) { status = messageFor(e) } } }, "—", {})
                }
            }
            else -> {
