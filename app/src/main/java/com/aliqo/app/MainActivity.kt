...
fun AuthScreen(onAuthenticated: (AuthResponse) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(24.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Button(
            ...
            enabled = !busy && email.isNotBlank() && password.length >= 8 && (!register || (username.length >= 3 && displayName.isNotBlank())),
            onClick = {
                busy = true
                status = "جارٍ الاتصال بالخادم... قد يستغرق أول اتصال قرابة دقيقة"
                scope.launch {
                    try {
                        val result = if (register) api.register(RegisterRequest(email, username, password, displayName)) else api.login(LoginRequest(email, password))
                        status = "تم بنجاح"
        ...
        OutlinedButton(Modifier.fillMaxWidth(), enabled = !busy, onClick = { register = !register; status = "" }) {
...
fun MainShell(token: String, refreshToken: String, onSignedOut: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            Button(Modifier.weight(1f), onClick = { tab = "home" }) { Text("الرئيسية") }
            Button(Modifier.weight(1f), onClick = { tab = "friends" }) { Text("الأصدقاء") }
            Button(Modifier.weight(1f), onClick = { tab = "profile" }) { Text("الملف") }
        }
        Spacer(Modifier.height(10.dp))
        if (status.isNotBlank()) Text(status)
        when (tab) {
            "friends" -> FriendsScreen(auth)
            "profile" -> ProfileScreen(auth, me, { reloadMe() }, refreshToken, onSignedOut)
            else -> HomeScreen(me)
        }
    }
}
...
fun FriendsScreen(auth: String) {
    ...
    var busy by remember { mutableStateOf(false) }
    val scope = rememberCoroutineScope()
    fun loadFriends() { scope.launch { try { friends = api.friends(auth) } catch (e: Exception) { status = messageFor(e) } } }
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Button(Modifier.fillMaxWidth(), enabled = !busy && query.length >= 2, onClick = {
            busy = true; scope.launch { try { results = api.searchUsers(auth, query); status = if (results.isEmpty()) "لا توجد نتائج" else "" } catch (e: Exception) { status = messageFor(e) }; busy = false }
        }) { Text("بحث") }
        if (status.isNotBlank()) Text(status)
        if (results.isNotEmpty()) Text("نتائج البحث", fontWeight = FontWeight.Bold)
        results.forEach { user ->
            UserCard(user, "إضافة", {
                scope.launch { try { val r = api.addFriend(auth, user.id); status = if (r.status == "ACCEPTED") "تمت إضافة @${user.username} كصديق" else "تم إرسال طلب الصداقة إلى @${user.username}"; loadFriends() } catch (e: Exception) { status = messageFor(e) } }
            }, "حظر", {
                scope.launch { try { api.blockUser(auth, user.id); results = results.filterNot { it.id == user.id }; status = "تم حظر @${user.username}"; loadFriends() } catch (e: Exception) { status = messageFor(e) } }
            })
        }
        ...
        friends.forEach { user ->
            UserCard(user, "حذف", {
                scope.launch { try { api.removeFriend(auth, user.id); status = "تم حذف @${user.username} من الأصدقاء"; loadFriends() } catch (e: Exception) { status = messageFor(e) } }
            }, "حظر", {
                scope.launch { try { api.blockUser(auth, user.id); status = "تم حظر @${user.username}"; loadFriends() } catch (e: Exception) { status = messageFor(e) } }
            })
        }
...
fun UserCard(user: UserDto, primaryLabel: String, onPrimary: () -> Unit, secondaryLabel: String, onSecondary: () -> Unit) {
    Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { Button(onClick = onPrimary) { Text(primaryLabel) }; OutlinedButton(onClick = onSecondary) { Text(secondaryLabel) } }
    } }
}
...
fun ProfileScreen(auth: String, me: UserDto?, onProfileUpdated: () -> Unit, refreshToken: String, onSignedOut: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        OutlinedTextField(displayName, { displayName = it }, Modifier.fillMaxWidth(), label = { Text("الاسم الظاهر") })
        OutlinedTextField(bio, { if (it.length <= 280) bio = it }, Modifier.fillMaxWidth(), minLines = 3, label = { Text("نبذة مختصرة (${bio.length}/280)") })
        Button(Modifier.fillMaxWidth(), enabled = displayName.isNotBlank(), onClick = {
            scope.launch { try { api.updateProfile(auth, UpdateProfileRequest(displayName.trim(), bio.trim())); status = "تم حفظ الملف الشخصي"; onProfileUpdated() } catch (e: Exception) { status = messageFor(e) } }
        }) { Text("حفظ التغييرات") }
        OutlinedButton(Modifier.fillMaxWidth(), onClick = {
            scope.launch { try { if (refreshToken.isNotBlank()) api.logout(mapOf("refreshToken" to refreshToken)) } catch (_: Exception) {}; onSignedOut() }
        }) { Text("تسجيل الخروج") }
        TextButton(Modifier.fillMaxWidth(), onClick = { confirmDelete = true }) { Text("حذف الحساب") }
        if (status.isNotBlank()) Text(status)
        Spacer(Modifier.height(30.dp))
    }
}
