...
private fun messageFor(e: Exception): String {
    if (e is HttpException) {
        val serverMessage = try { e.response()?.errorBody()?.string()?.take(180) } catch (_: Exception) { null }
        return when (e.code()) {
            400 -> "الطلب غير مقبول${serverMessage?.let { ": $it" } ?: ""}"
...
fun AliqoApp() {
    ...
    if (accessToken.isBlank()) {
        ...
        MainShell(
            ...
            onTokensUpdated = { a, r -> saveTokens(a, r) },
                ...
                scope.launch {
                    try {
                        val result = if (register) {
                            api.register(RegisterRequest(email, username, password, displayName.trim()))
                        } else api.login(LoginRequest(email, password))
            ...
            try {
                ...
                if (e is HttpException && e.code() == 401 && refreshSession()) {
                    try {
                        me = api.me("Bearer $currentAccess")
                        status = ""
                    } catch (retry: Exception) { status = messageFor(retry); onSignedOut() }
                } else {
                    status = messageFor(e)
            ...
            "requests" -> {
                ...
                requests.forEach { request ->
                    UserCard(
                        user = request.user,
                        primaryLabel = "قبول",
                        onPrimary = { scope.launch { try { api.acceptFriend(auth, request.id); status = "تم قبول @${request.user.username}"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } },
                        secondaryLabel = "رفض",
                        onSecondary = { scope.launch { try { api.rejectFriend(auth, request.id); status = "تم رفض الطلب"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } }
                    )
                }
            }
            "blocked" -> {
                ...
                blocked.forEach { user ->
                    UserCard(
                        user = user,
                        primaryLabel = "إلغاء الحظر",
                        onPrimary = { scope.launch { try { api.unblockUser(auth, user.id); status = "تم إلغاء حظر @${user.username}"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } },
                        secondaryLabel = "—",
                        onSecondary = {}
                    )
                }
                ...
                results.forEach { user ->
                    UserCard(
                        user = user,
                        primaryLabel = "إضافة",
                        onPrimary = { scope.launch { try { val r = api.addFriend(auth, user.id); status = if (r.status == "ACCEPTED") "أصبحتما صديقين" else "تم إرسال طلب الصداقة"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } },
                        secondaryLabel = "حظر",
                        onSecondary = { scope.launch { try { api.blockUser(auth, user.id); results = results.filterNot { it.id == user.id }; status = "تم حظر @${user.username}"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } }
                    )
                }
                ...
                friends.forEach { user ->
                    UserCard(
                        user = user,
                        primaryLabel = "حذف",
                        onPrimary = { scope.launch { try { api.removeFriend(auth, user.id); status = "تم حذف @${user.username}"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } },
                        secondaryLabel = "حظر",
                        onSecondary = { scope.launch { try { api.blockUser(auth, user.id); status = "تم حظر @${user.username}"; reloadAll() } catch (e: Exception) { status = messageFor(e) } } }
                    )
                }
            }
        }
...
fun ProfileScreen(
        ...
        AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = {
                    confirmDelete = false
                    scope.launch {
                        try { api.deleteAccount(auth); onSignedOut() }
                        catch (e: Exception) { status = messageFor(e) }
                    }
                }) { Text("حذف نهائي") }
            },
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        OutlinedButton(
            onClick = {
                scope.launch {
                    try { if (refreshToken.isNotBlank()) api.logout(RefreshRequest(refreshToken)) } catch (_: Exception) {}
                    onSignedOut()
                }
            },
            modifier = Modifier.fillMaxWidth()
