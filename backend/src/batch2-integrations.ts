...
export class Batch2Integrations {
  ...
  async prepareMedia(userId: string, input: PrepareMediaInput) {
    ...
    const row = await prisma.mediaUpload.create({
      data: {
        ...
        mimeType: input.mimeType.trim().toLowerCase().slice(0, 120),
        byteSize: input.byteSize,
  ...
  async markUploaded(userId: string, uploadId: string) {
    ...
    if (row.expiresAt <= new Date()) {
      await prisma.mediaUpload.update({ where: { id: uploadId }, data: { status: MediaUploadStatus.EXPIRED } });
    ...
    return prisma.mediaUpload.update({
      where: { id: uploadId },
      data: { status: MediaUploadStatus.UPLOADED, publicUrl, uploadedAt: new Date() },
    });
  }

  async consumeUploaded(userId: string, chatId: string, uploadId: string) {
    const row = await prisma.mediaUpload.findFirst({
      where: { id: uploadId, userId, chatId, status: MediaUploadStatus.UPLOADED },
    });
    if (!row || !row.publicUrl) throw new Error('UPLOAD_NOT_READY');
    const updated = await prisma.mediaUpload.updateMany({
      where: { id: row.id, status: MediaUploadStatus.UPLOADED },
      data: { status: MediaUploadStatus.ATTACHED, attachedAt: new Date() },
    });
  ...
  async pushToUser(userId: string, title: string, body: string, data: Record<string, string> = {}) {
    const devices = await prisma.deviceToken.findMany({
      where: { userId, revokedAt: null },
      select: { id: true, tokenValue: true },
    });
...
export const inferMessageType = (kind: MediaKind) =>
  kind === 'IMAGE' ? 'IMAGE' : kind === 'VIDEO' ? 'VIDEO' : kind === 'VOICE' ? 'VOICE' : 'FILE';
