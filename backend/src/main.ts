...
class ChatsController {
  ...
  @Post(':chatId/messages') async send(@Req() req: any, @Param('chatId') chatId: string, @Body() dto: SendMessageDto) { await this.member(chatId, req.user.id); const type = dto.type || MessageType.TEXT; if (type !== MessageType.TEXT) throw new BadRequestException('Use secure media upload flow'); const text = dto.text?.trim(); if (!text) throw new BadRequestException('Text required'); if (dto.replyToId && !await prisma.message.findFirst({ where: { id: dto.replyToId, chatId, deletedAt: null } })) throw new BadRequestException('Invalid reply'); const message = await prisma.message.create({ data: { chatId, senderId: req.user.id, type: MessageType.TEXT, text, replyToId: dto.replyToId || null }, include: { sender: { select: publicUserSelect }, replyTo: { select: { id: true, text: true, type: true, senderId: true } }, reactions: true } }); await prisma.chat.update({ where: { id: chatId }, data: { updatedAt: new Date() } }); this.realtime.emitChat(chatId, 'message:new', message); const name = message.sender.profile?.displayName || message.sender.username; await this.notifyMembers(chatId, req.user.id, name, text); return message; }
