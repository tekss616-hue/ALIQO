...
class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    if (!user || !user.isActive || user.deletedAt || !(await argon2.verify(user.passwordHash, dto.password))) {
  ...
  async refresh(raw: string) {
    ...
    if (!row || row.revokedAt || row.expiresAt <= new Date() || !row.user.isActive || row.user.deletedAt) {
  ...
  async forgotPassword(emailInput: string) {
    ...
    await prisma.passwordResetToken.create({
      data: { userId: user.id, tokenHash: hashToken(raw), expiresAt: new Date(Date.now() + 15 * 60000) }
    });
  ...
  @Get('search') async search(@Req() req: any, @Query('q') q = '') {
    ...
    return prisma.user.findMany({
      ...
      select: {
        id: true,
        username: true,
        profile: { select: { displayName: true, avatarUrl: true, bio: true, isOnline: true, lastSeenAt: true } }
      },
      take: 20
    });
  }
    ...
    await prisma.$transaction([
      ...
      prisma.user.update({
        ...
        data: {
          ...
          username: `deleted_${req.user.id.slice(-12)}`
        }
      })
    ]);
    return { ok: true };
...
class FriendsController {
  ...
  @Get() async list(@Req() req: any) {
    ...
    return rows.map(r => this.publicUser(r.requesterId === req.user.id ? r.addressee : r.requester));
  }
  ...
  @Post(':userId/request') async request(@Req() req: any, @Param('userId') target: string) {
    ...
    const blocked = await prisma.blockedUser.findFirst({
      where: { OR: [{ blockerId: req.user.id, blockedId: target }, { blockerId: target, blockedId: req.user.id }] }
    });
    ...
    const accepted = await prisma.friendship.findFirst({
      where: {
        status: FriendshipStatus.ACCEPTED,
        OR: [
          { requesterId: req.user.id, addresseeId: target },
          { requesterId: target, addresseeId: req.user.id }
        ]
      }
    });
    if (accepted) return accepted;

    const inverse = await prisma.friendship.findUnique({
      where: { requesterId_addresseeId: { requesterId: target, addresseeId: req.user.id } }
    });
    if (inverse?.status === FriendshipStatus.PENDING) {
      return prisma.friendship.update({ where: { id: inverse.id }, data: { status: FriendshipStatus.ACCEPTED } });
    }

    return prisma.friendship.upsert({
      where: { requesterId_addresseeId: { requesterId: req.user.id, addresseeId: target } },
      create: { requesterId: req.user.id, addresseeId: target },
  ...
  @Post(':requestId/accept') async accept(@Req() req: any, @Param('requestId') id: string) {
    const row = await prisma.friendship.findFirst({
      where: { id, addresseeId: req.user.id, status: FriendshipStatus.PENDING }
    });
...
async function bootstrap() {
  if (!process.env.JWT_ACCESS_SECRET || process.env.JWT_ACCESS_SECRET.length < 32) {
    throw new Error('JWT_ACCESS_SECRET must be set to a strong secret');
  }
